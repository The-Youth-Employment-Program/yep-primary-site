...
export function Testimonials(props: PageSectionTestimonials) {
  ...
  let mainstyle = `grid py-14 px-8 sm:p-10 md:p-20 lg:px-40 items-center content-center rounded-lg ${props.bg_color} ${props.text_color}`
  let secondarystyle = `pl-4 ${props.accent_color}`
  let quoteColor = `${props.accent_color}`
    return(
      <>
        <section className={mainstyle}>
          {props.testimonial?.map((testimony,i) => {
            return( // Animate each one to scroll after delay?
              <div key={i} data-tina-field={tinaField(props[i],"testimonial")} className="lg:text-2xl sm:text-center mb-14 last:mb-0"> {/*TinaField doesn't work */}
