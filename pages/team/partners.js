            ...
            <div className="grid justify-items-center grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 3xl:grid-cols-4"> {/* xl:grid-flow-col / grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 */}
              {data.teamConnection.edges.map((part, i) =>{
                  switch (part?.node.__typename) {
                      case "TeamPartner" : {
                        return <Partner key={i} {...part.node} />
                          // return (
                          ...
                          //                 <div className="flex flex-shrink-0 h-64 sm:h-96 aspect-square">
                          //                 { isOpen && <img onClick={toggleIsOpen} src={part?.node.image} alt="" className="object-cover rounded-lg dark:bg-gray-500 aspect-square cursor-pointer" />}
                          //                 { !isOpen && <p onClick={toggleIsOpen} className="flex w-full text-justify items-center rounded-lg cursor-pointer aspect-square">{part?.node.blurb}</p>}
                          //                 </div>
                          //                 <div>
