...
<div className="grid mx-2 mb-4 sm:grid-cols-1 sm:gap-4 sm:mb-4 md:gap-8 md:mb-8 lg:gap-14">
  ...
  if (job?.node?.active) {
          switch (job?.node?.__typename) {
            case "JobsFulltime" : {
              return (
                <>
                <div key={i} className="pb-6 h-full rounded-lg shadow-md"> {/* grid md:grid-cols-2 pb-3 */}
                    <div className="relative p-0 m-0"> {/* data */}
                      ...
                      <p className="line-clamp-2 px-8">{job.node.brief}</p>
            ...
            case "JobsSeasonal" : {
              return (
                <>
                <div key={i} className="pb-6 h-full rounded-lg shadow-md"> {/* grid md:grid-cols-2 pb-3 */}
                    <div className="relative pt-0 pb-2 m-0"> {/* data */}
                      <div className="flex justify-between align-middle py-4">   
                        <a href={`/jobs/${job?.node._sys.filename}` || "#"}>
                          <h2 className="ml-4 mt-2 align-middle text-xl font-medium text-salmon-dark-g1" data-tina-field={tinaField(jobsList[i].node,"title")}>
                            { job.node.title }
